...
class ProductScreen extends SourceConsumerStatefulWidget {
  ...
  const ProductScreen({super.key, required String this.productId, this.order, this.orderItem})
    : cartId = null,
      cartItemId = null;

  const ProductScreen.fromCart({super.key, required String this.cartItemId})
    : productId = null,
      order = null,
      orderItem = null,
      cartId = Env.cartId;
...
class _ProductScreenState extends SourceConsumerState<ProductScreen> {
  ...
  Future<void> _init() async {
    final (:user, :product, :carts, :cart, :cartItem) = await ref.futureOfData(_provider);
    ...
    _levelsFb.addAll({
      for (final ingredient in levels)
        ingredient.id: FormControlTyped(initialValue: resolveInitialIngredientValue(ingredient)),
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: state.buildView(
        ...
        data: (items) =>
            _buildBody(items.product.addableIngredients, items.product.levels, items.carts),
      ),
    );
  }
  ...
  Widget _buildBody(
    ...
    Widget buildCartField() {
      return FieldPadding(
        ReactiveDropdownField(
          ...
          items: carts.map((e) {
            return DropdownMenuItem(value: e, child: Text(e.displayTitle));
          }).toList(),
        ),
      );
    }
    ...
    Widget buildBuyersField(IList<UserDto> users) {
      return FieldPadding(
        ReactivePopupMenuButton<UserDto>.withChip(
